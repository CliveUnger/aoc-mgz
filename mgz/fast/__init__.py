...
from mgz.fast.actions import ActionPayload, parse_action_71094
from mgz.fast.enums import Action, Age, Operation, Postgame
from mgz.util import check_flags

__all__ = ["Action", "ActionPayload", "Age", "Operation", "Postgame"]
...
def sync(data):
    ...
    (
        checksum,
        is_de,
    ) = struct.unpack("<4xI4xI", data.read(16))
    if not is_de:
        data.read(8)
...
def parse_action(action_type: Action, data: bytes) -> ActionPayload:
    ...
    if action_type == Action.DE_TRIBUTE:
        player_id, player_id_to, unknown_byte, unknown_int, wood, food, gold, stone = struct.unpack_from(
            "<bbbiffff", data
        )
        return dict(player_id=player_id, player_id_to=player_id_to, food=food, wood=wood, stone=stone, gold=gold)
    ...
    if action_type == Action.SPECIAL:
        ...
        values = ActionPayload(object_ids=list(object_ids), order_id=order_id)
        if x > 0 and y > 0:
...
def action(data, sequence: int | None = None) -> tuple[Action, ActionPayload]:
    """Handle actions."""
        ...
        if postgame_type == Postgame.WORLD_TIME:
            ...
            for lb in range(0, num_leaderboards):
                ...
                for i in range(0, num_players):
                    ...
                    player_data.append({"number": player_num, "rank": rank, "rating": rating})
                leaderboards.append({"id": leaderboard_id, "players": player_data})
